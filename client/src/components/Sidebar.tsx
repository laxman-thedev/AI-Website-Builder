...
const Sidebar = ({
    ...
    return (
        <div
            className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border border-gray-500 transition-all ${isMenuOpen ? "max-sm:w-0 overflow-hidden" : "w-full"
                }`}
        >
                        ...
                        .map((message) => {
                            ...
                            if (isMessage) {
                                ...
                                return (
                                    <div
                                        key={msg.id}
                                        className={`flex items-start gap-3 my-3 ${isUser ? "justify-end" : "justify-start"
                                            }`}
                                    >
                                        {!isUser && (
                                            <div className="w-8 h-8 rounded-full bg-linear-to-br from-indigo-700 to-indigo-800 flex items-center justify-center">
                                                <BotIcon className="size-5 text-white" />
                                            </div>
                                        )}

                                        <div
                                            className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm leading-relaxed ${isUser
                                                    ? "bg-linear-to-br from-indigo-500 to-indigo-600 text-white rounded-tr-none"
                                ...
                                return (
                                    <div
                                        key={ver.id}
                                        className="max-w-[85%] w-[80%] mx-auto my-3 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2"
                                    >
                                        ...
                                        <div className="flex items-center justify-between">
                                            {project.current_version_index === ver.id ? (
                                                <button
                                                    ...
                                                    className="px-3 py-1 rounded-md text-xs bg-gray-700"
                                                >
                                                    Current version
                                                </button>
                                            ) : (
